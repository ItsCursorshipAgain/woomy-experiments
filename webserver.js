...
if (TURN_SECRET === "" || TURN_SERVER_URI === "turn:127.0.0.1:3048") {
  console.warn(
    `[CRITICAL WARNING] Invalid or missing TURN_SECRET (${TURN_SECRET}) or TURN_SERVER_URI (${TURN_SERVER_URI})`,
  );
}
  ...
  if (pathname.startsWith("/api/")) {
    ...
    if (pathname === "/api/list") {
      ...
      res.end(
        JSON.stringify(
          fs
            .readdirSync(path.join(__dirname, "client", "js"), {
              recursive: true,
            })
  ...
  switch (type) {
    case "host":
      ...
      function sendHeartbeat() {
        if (ws.readyState !== 1) {
          console.log(
            `Failed to send hearbeat to room ${room.id} (readyState = ${ws.readyState})`,
          );
        } else {
        ...
        try {
          ...
          if (maxPlayers !== undefined)
            room.maxPlayers = Math.min(
              99,
              Math.max(1, Number(maxPlayers) || 99),
            );
    ...
    default:
      console.log(
        `[WebSocket] Unknown connection type "${type}". Terminating.`,
      );
      ws.terminate();
      break;
  }
